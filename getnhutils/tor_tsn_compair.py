...
def get_ovsdb_dump(hostname='',port=''):
    tor = GetOvsdbColumns(hostname=hostname,port=port)
    try:
        tor.open_socket()
...
def check_active_tor(tor_name,tor_list):
    for i in tor_list:
        tor_ip_port = i.split(':')
        ...
        if type(ovsdb_pswitch['OvsdbPhysicalSwitchResp']['pswitch']) == list:
            ...
            if tor_name == tsn_tor_name:
                return tor_name,tor_ip_port
            else:
                msg = 'KeyError: Could not match ToR_Name %s' % tor_name
                raise msg
    msg = 'KeyError: Could not match ToR_Name %s' % tor_name
    raise msg

def get_ovsdb_table(dump_data,ovsdb_key_list):
    ovsdb_table_list = {}
...
def get_tsn_table(tsn_ip_port,tsn_key_list):
    tsn = GetContrailSandesh(hostname=tsn_ip_port[0],port=tsn_ip_port[1])
    tsn_table_list = {}
    for k in tsn_key_list:
        exe_txt = 'data = tsn.%s()' % (tsn_key_list[k])
        exec(exe_txt)
        tsn_table_list[k] = data
    return tsn_table_list

def get_lsw_uuid(ovsdb_table,logical_switch):
    lsw_uuid = None
...
def get_locator_dst_ip(ovsdb_table,locator_set):
...
def get_locator_set_dst_ip(ovsdb_table,locator_set):
    locator_uuid = None
    for i in ovsdb_table['Physical_Locator']:
        if i['_uuid'][1] == locator_set:
...
def compair_tables(tsn_tables,ovsdb_tables):
    data_list = []
    for i in tsn_tables:
        flag = True
...
def chk_physical_switch(tsn_table,ovsdb_table):
    tsn_table = tsn_table['tsn_psw']['OvsdbPhysicalSwitchResp']['pswitch'][0]
...
def chk_logical_switch(tsn_table,ovsdb_table):
    ...
    chk_list = compair_tables(tsn_lsw_list,ovsdb_lsw_list)
    return chk_list

def chk_mcast_macs_local(tsn_table,ovsdb_table):
    tor_tunnel_ip = tsn_table['tsn_psw']['OvsdbPhysicalSwitchResp']['pswitch'][0]['tunnel_ip']
    ...
    for i in ovsdb_table['Mcast_Macs_Local']:
        logical_switch = get_lsw_uuid(ovsdb_table,i['logical_switch'][1])
        locator_set = get_locator_set_dst_ip(ovsdb_table,i['locator_set'][1])
    ...
    chk_list = compair_tables(tsn_mc_mac_list,ovsdb_mc_mac_list)
    return chk_list

def chk_mcast_macs_remote(tsn_table,ovsdb_table):
    lswitch_table = tsn_table['tsn_lsw']['OvsdbLogicalSwitchResp']['lswitch']
    tor_ip = tsn_table['tsn_client']['OvsdbClientResp']['client'][0]['tor_service_node']
    tsn_table = tsn_table['tsn_mc_mac_local']['OvsdbMulticastMacLocalResp']['macs']
    tsn_mc_mac_list = []
    for i in tsn_table:
...
def chk_physical_port(tsn_table,ovsdb_table):
    ...
    for i in ovsdb_table['Physical_Port']:
        ...
        if len(i['vlan_bindings'][1]) != 0:
            for j in i['vlan_bindings'][1]:
                lsw_uuid = get_lsw_uuid(ovsdb_table,j[1][1])
                vlan = [str(j[0]),lsw_uuid]
                vlan_list.append(vlan)
        pport_list = [vlan_list,i['name']]
        ovsdb_pport_list.append(pport_list)
    chk_list = compair_tables(tsn_tables,ovsdb_tables)
    return chk_list

def chk_ucast_macs_local(tsn_table,ovsdb_table):
    ...
    for i in ovsdb_table['Ucast_Macs_Local']:
        logical_switch = get_lsw_uuid(ovsdb_table,i['logical_switch'][1])
        dst_ip = get_locator_dst_ip(ovsdb_table,i['locator'][1])
        uc_local_mac_list = [i['MAC'],logical_switch,dst_ip]
        ovsdb_uc_local_mac_list.append(uc_local_mac_list)
    chk_list = compair_tables(tsn_uc_local_mac_list,ovsdb_uc_local_mac_list)
    return chk_list

def chk_ucast_macs_remote(tsn_table,ovsdb_table):
    remote_macs_list = []
    for i in tsn_table['tsn_lsw']['OvsdbLogicalSwitchResp']['lswitch']:
        lsw_uuid = i['name']
        remote_macs = tsn.get_ovsdb_unicast_mac_remote(logical_switch=lsw_uuid)
        remote_macs = remote_macs['OvsdbUnicastMacRemoteResp']['macs']
        if len(remote_macs) != 0:
            for j in remote_macs:
                if j['dest_ip'] != None and j['self_exported'] == 'false':
    ...
    chk_list = compair_tables(tsn_uc_remote_mac_list,ovsdb_uc_remote_mac_list)
    return chk_list

def get_tsn_ovsdb_dump(tsn_conn_list,tor_conn):
    tor_conn_data = tor_conn.split(':')
    dump_data = get_ovsdb_dump(hostname=tor_conn_data[0],port=int(tor_conn_data[1]))
    tor_name = dump_data['Physical_Switch']['result'][0]['rows'][0]['name']
    tor_name,tsn_ip_port = check_active_tor(tor_name,tsn_conn_list)
    ...
    tsn_table = get_tsn_table(tsn_ip_port,tsn_key_list)
    ovsdb_table = get_ovsdb_table(dump_data,ovsdb_key_list)
