...
class GetContrailSandesh(object):
    ...
    def get_itf(self, name='', _type='', uuid='', vn='', mac='', ipv4_address='', ipv6_address='', parent_uuid='', ip_active='', ip6_active='', l2_active=''):
        ...
        path = 'Snh_ItfReq?name=%s&type=%s&uuid=%s&vn=%s&mac=%s&ipv4_address=%s&ipv6_address=%s&parent_uuid=%s&ip_active=%s&ip6_active=%s&l2_active=%s' % (
            name, _type, uuid, vn, mac, ipv4_address, ipv6_address, parent_uuid, ip_active, ip6_active, l2_active)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_ovsdb_physical_switch(self, session_remote_ip='', session_remote_port=''):
        ...
        path = 'Snh_OvsdbPhysicalSwitchReq?session_remote_ip=%s&session_remote_port=%s' % (
            session_remote_ip, session_remote_port)
        return self.get_path_sandesh_to_dict(path)

    def get_ovsdb_physical_port(self, session_remote_ip='', session_remote_port='', name=''):
        # Return OvsdbPhysicalPortResp by dict
        path = 'Snh_OvsdbPhysicalPortReq?session_remote_ip=%s&session_remote_port=%s&name=%s' % (
            session_remote_ip, session_remote_port, name)
        return self.get_path_sandesh_to_dict(path)

    def get_ovsdb_logical_switch(self, session_remote_ip='', session_remote_port='', name=''):
        # Return OvsdbLogicalSwitchResp by dict
        path = 'Snh_OvsdbLogicalSwitchReq?session_remote_ip=%s&session_remote_port=%s&name=%s' % (
            session_remote_ip, session_remote_port, name)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_ovsdb_vlan_port_binding(self, session_remote_ip='', session_remote_port='', physical_port=''):
        ...
        path = 'Snh_OvsdbVlanPortBindingReq?session_remote_ip=%s&session_remote_port=%s&physical_port=%s' % (
            session_remote_ip, session_remote_port, physical_port)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_ovsdb_unicast_mac_local(self, session_remote_ip='', session_remote_port='', logical_switch='', mac=''):
        ...
        path = 'Snh_OvsdbUnicastMacLocalReq?session_remote_ip=%s&session_remote_port=%s&logical_switch=%s&mac=%s' % (
            session_remote_ip, session_remote_port, logical_switch, mac)
        return self.get_path_sandesh_to_dict(path)

    def get_ovsdb_vrf(self, session_remote_ip='', session_remote_port='', logical_switch='', mac=''):
        # Return OvsdbVrfResp by dict
        path = 'Snh_OvsdbVrfReq?session_remote_ip=%s&session_remote_port=%s&logical_switch=%s&mac=%s' % (
            session_remote_ip, session_remote_port, logical_switch, mac)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_ovsdb_unicast_mac_remote(self, session_remote_ip='', session_remote_port='', logical_switch='', mac=''):
        ...
        path = 'Snh_OvsdbUnicastMacRemoteReq?session_remote_ip=%s&session_remote_port=%s&logical_switch=%s&mac=%s' % (
            session_remote_ip, session_remote_port, logical_switch, mac)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_ovsdb_multicast_mac_local(self, session_remote_ip='', session_remote_port='', logical_switch=''):
        ...
        path = 'Snh_OvsdbMulticastMacLocalReq?session_remote_ip=%s&session_remote_port=%s&logical_switch=%s' % (
            session_remote_ip, session_remote_port, logical_switch)
        return self.get_path_sandesh_to_dict(path)

    def get_ovsdb_ha_stale_dev_vn_export(self, dev_name='', vn_uuid=''):
        # Return OvsdbHaStaleDevVnExportResp by dict
        path = 'Snh_OvsdbHaStaleDevVnExportReq?dev_name=%s&vn_uuid=%s' % (
            dev_name, vn_uuid)
        return self.get_path_sandesh_to_dict(path)

    def get_ovsdb_ha_stale_l2route_export(self, dev_name='', vn_uuid='', mac=''):
        # Return OvsdbHaStaleL2RouteExportReq by dict
        path = 'Snh_OvsdbHaStaleL2RouteExportReq?dev_name=%s&vn_uuid=%s&mac=%s' % (
            dev_name, vn_uuid, mac)
        return self.get_path_sandesh_to_dict(path)
    ...
    def get_pkt_trace_info(self, num_buf='', fnum_buf=''):
        ...
        path = 'Snh_PktTraceInfo?num_buffers=%s&flow_num_buffers=%s' % (
            num_buf, fnum_buf)
        return self.get_path_sandesh_to_dict(path)
